...
export async function POST(req: NextRequest) {
  try {
    ...
    const dateValue = (body.date?.trim() || formatDate(new Date())).slice(0, 30);
    ...
    try {
      ...
      return NextResponse.json(
        { error: (envError as Error).message || 'Missing account configuration for business' },
        { status: 500 }
      );
    }
